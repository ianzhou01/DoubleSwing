...
void DoublePendulum::integrateBoth_RK4(float dt) {
    ...
    float k1_accel1 = calculate_accel_p1(p1.theta, p1.omega, p2.theta, p2.omega);
    float k1_accel2 = calculate_accel_p2(p1.theta, p1.omega, p2.theta, p2.omega);

    // k2
    float k2_theta1 = p1.theta + 0.5 * k1_omega1 * dt;
    float k2_theta2 = p2.theta + 0.5 * k1_omega2 * dt;
    ...
    // k3
    float k3_theta1 = p1.theta + 0.5 * k2_omega1 * dt;
    float k3_theta2 = p2.theta + 0.5 * k2_omega2 * dt;
    ...
    // k4
    float k4_theta1 = p1.theta + k3_omega1 * dt;
    float k4_theta2 = p2.theta + k3_omega2 * dt;
    ...
    p1.theta += (dt / 6.0) * (k1_omega1 + 2 * k2_omega1 + 2 * k3_omega1 + k4_omega1);
    p1.omega += (dt / 6.0) * (k1_accel1 + 2 * k2_accel1 + 2 * k3_accel1 + k4_accel1);

    p2.theta += (dt / 6.0) * (k1_omega2 + 2 * k2_omega2 + 2 * k3_omega2 + k4_omega2);
...
void DoublePendulum::integrateP2_RK4(float dt) {
    ...
    float k2_theta2 = p2.theta + 0.5 * k1_omega2 * dt;
    float k2_omega2 = p2.omega + 0.5 * k1_accel2 * dt;
    float k2_accel2 = calculate_accel_p2(p1.theta, p1.omega, k2_theta2, k2_omega2);

    // k3
    float k3_theta2 = p2.theta + 0.5 * k2_omega2 * dt;
    float k3_omega2 = p2.omega + 0.5 * k2_accel2 * dt;
    float k3_accel2 = calculate_accel_p2(p1.theta, p1.omega, k3_theta2, k3_omega2);

    // k4
    float k4_theta2 = p2.theta + k3_omega2 * dt;
    float k4_omega2 = p2.omega + k3_accel2 * dt;
    float k4_accel2 = calculate_accel_p2(p1.theta, p1.omega, k4_theta2, k4_omega2);
...
void DoublePendulum::updateBoth() {
    ...
    Utility::normalize_angle(p1.theta);
    Utility::normalize_angle(p2.theta);
...
void DoublePendulum::draw(sf::RenderWindow &win, int centerX, int centerY, bool paused) {
    ...
    float p1X = centerX - PX_METER_RATIO * p1.len * sin(p1.theta);
    float p1Y = centerY + PX_METER_RATIO * p1.len * cos(p1.theta);
